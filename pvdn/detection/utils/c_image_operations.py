...
class Cblob(object):
    ...
    def __init__(self, k=0.06, w=11, eps=1e-3, n_p=None, nms_distance=2, padding=5):
        """
        Constructur method.
    ...
    def _init_c_lib(self):
        ...
        lib_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "image_operations.so")
        ...
        self.__c_binarize.argtypes = [ndpointer(ctypes.c_float, flags="C_CONTIGUOUS"),
        ...
        self.__c_find_proposals.argtypes = [ndpointer(ctypes.c_int, flags="C_CONTIGUOUS"),
                                            ndpointer(ctypes.c_ubyte, flags="C_CONTIGUOUS"),
    ...
    def binarize_in_c(self, img):
        ...
        if not "float" in str(img.dtype):
            raise TypeError("The input image is expected to be of type np.float32 or np.float64, "
        ...
        self.__c_binarize(img, bin_img, ii, ctypes.c_int(h), ctypes.c_int(w), ctypes.c_int(self.window),
    ...
    def find_proposals_in_c(self, bin_img):
        ...
        if not "uint8" in str(bin_img.dtype):
            raise TypeError("The binary input image is expected to be of type np.uint8, "
        ...
        # c function
        self.__c_find_proposals(final_proposals, bin_img, ctypes.c_int(h), ctypes.c_int(w),
                                ctypes.c_int(self.padding), ctypes.c_int(self.nms_distance))
...
LIB_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), "image_operations.so")
...
def find_proposals(bin_img, padding, nms_distance):
    ...
    # c function
    _find_proposals = (final_proposals, bin_img, ctypes.c_int(h), ctypes.c_int(w),
...
def binarize(img, k=0.06, window=11, eps=1e-3):
    ...
    _binarize(img, bin_img, ii, ctypes.c_int(h), ctypes.c_int(w), ctypes.c_int(window),
